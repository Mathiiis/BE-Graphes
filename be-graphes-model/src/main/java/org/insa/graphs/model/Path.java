...
public class Path {
        ...
        for (int i = 0; i < nodes.size() - 1; i++) {
            ...
            for (Arc arc : successors) {
                if (arc.getDestination().equals(nextNode)) {
                    if (fastest == null || arc.getMinimumTravelTime() < fastest.getMinimumTravelTime()) {
                        fastest = arc;
                    }
                }
            }
    ...
    public static Path createShortestPathFromNodes(Graph graph, List<Node> nodes)
        ...
        for (int i = 0; i < nodes.size() - 1; i++) {
            ...
            for (Arc arc : successors) {
                if (arc.getDestination().equals(nextNode)) {
                    if (shortest == null || arc.getLength() < shortest.getLength()) {
                        shortest = arc;
                    }
                }
            }
    ...
    public double getMinimumTravelTime() {
        double minimumTravelTime = 0.0;
        for (Arc arc : arcs) {
            minimumTravelTime += arc.getTravelTime(arc.getRoadInformation().getMaximumSpeed());
        }
